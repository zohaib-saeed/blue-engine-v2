...
const FeaturedSection: React.FC<ISubFeatureSectionDataType> = ({ data, index }) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <h3 className="font-extrabold text-[28px] leading-8 whitespace-break-spaces">{title}</h3>
        <p className="font-normal text-base whitespace-break-spaces">{subText}</p>
      </div>
      <div
        className={cn(
      ...
      >
        <img src={image} alt="featured-section" className="w-full h-full absolute top-0 left-0" />
      </div>
    </section>
  );
};
